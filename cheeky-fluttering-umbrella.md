...
```bash
cd ./grimar && bun run check
cd ./grimar && bun run vitest run
```

## Migration Strategy

1. **Backup existing database** before schema changes
2. **Phase 1-3**: Can be done without DB changes (safe)
3. **Phase 4**: Requires DB migration - create new columns, populate jsonPath
4. **Phase 5-6**: After data migration is complete

## Questions for User

1. **Data file location**: Should external JSON files be in `data/` (project root) or `static/data/` (served publicly)?
2. **One file per item vs all-in-one**: Preference for Option A (one JSON per type) or Option B (one JSON per item)?
