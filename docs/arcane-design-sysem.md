...
**Version:** 1.0.0
**Theme Philosophy:** "Modern Arcane Aero." A modern UI foundation (clean spacing
, clarity, accessibility) with selective Frutiger Aero influences (gloss
reflections, beveled glass depth, gemstone accents).

**Document Roles:**

* This document is the **authoritative** source for tokens, states, motion
  rules, accessibility, and component conventions.
* `docs/STYLE_GUIDE.md` provides **visual recipes** (shine/noise/bevel
  examples) that should be applied through these tokens.
...
* **If they disagree:**
  ...
  * The Style Guide may propose a look, but it must map cleanly onto tokens and
  states defined here.
...
* **Depth via layers, not noise:** Use the 3 glass materials instead of
inventing new surface styles.
* **Motion communicates state:** Animations exist to confirm interactions and
state changes.
* **Consistent semantics:** Prefer named meanings (primary/secondary, canvas/
card/overlay, rarity tiers) over one-off styling.
* **Accessible by default:** Keyboard focus is always visible, and
...
* **Variants:** use human names over numbers (e.g. `primary`, `secondary`
, `ghost`, `danger`)
...
We use **Inter** for its clean legibility on screens, vital for reading dense
spell descriptions.
...
* **Color:** match semantic text (`text-gray-300`) unless representing
rarity/semantic state
...
Rarity should be expressed primarily through border intensity + subtle glow, not
...
(Prefer `animate-pulse-slow` for featured items)

### 1.3 Semantic Color Mapping (Tokens)

These are the “default answers” so you don’t have to invent colors each time.
Prefer semantic tokens over ad-hoc colors.
...
* **Hover is not a requirement:** any hover-only affordance must have a
non-hover equivalent.
...
* **Light mode:** keep the same hierarchy but lighten surfaces and reduce blur
intensity if readability suffers.
...
To maintain depth, we define three specific "materials" based on hierarchy.
Avoid solid opaque backgrounds.
...
* **Primary (Cast Spell):**
  ...
  ```html
  <button class="bg-linear-to-r from-purple-600 to-indigo-600 text-white
  font-bold py-2 px-6 rounded-xl shadow-lg shadow-purple-500/30 hover:scale-105
...
* **Secondary (Cancel/Back):**
  ```html
  <button class="bg-white/5 hover:bg-white/10 text-gray-300 border
  border-white/10 py-2 px-6 rounded-xl backdrop-blur-sm transition-all">
    Cancel
  </button>
  ```
...
* **Base Style:**
  `bg-gray-900/50 border border-purple-500/30 text-white rounded-xl focus:ring-2
focus:ring-purple-500/50 focus:border-purple-500 placeholder-gray-500
...
* **App shell background:**
  `min-h-dvh bg-linear-to-br from-slate-900 via-indigo-950 to-slate-900 text-gray-100`
...
* **Primary button (Cast Spell):**
  `bg-linear-to-r from-purple-600 to-indigo-600 text-white font-bold py-2 px-6
rounded-xl shadow-lg shadow-purple-500/30 hover:scale-105 active:scale-95`

* **Secondary button:**
  `bg-white/5 hover:bg-white/10 text-gray-300 border border-white/10 py-2 px-6
rounded-xl backdrop-blur-sm`

* **Input (Crystal):**
  `bg-gray-900/50 border border-purple-500/30 text-white rounded-xl placeholder
-gray-500 backdrop-blur-sm focus-visible:ring-2 focus-visible:ring-purple-500/60
focus-visible:outline-none focus:border-purple-500`
...
* **Typography:** use the Tailwind typography plugin (`prose`) if helpful, but
keep it themed
...
Motion should feel like “arcane UI feedback,” not like a game UI. Prefer subtle
transforms + opacity. Avoid animating layout where possible.
...
* **Use sparingly:** `box-shadow` (keep subtle), `backdrop-blur` (avoid
...
* **Empty/Spent:** A dull, grayed-out icon.
  * *Color:* `text-gray-700`
* **Interaction:** Clicking "breaks" the gem (scales down instantly `scale-75`
then back up).
...
* **Effect:** Burst of particles in the color of the damage type (Fire = Orange,
Cold = White/Blue).
  ...
  theme: {
    extend: {
      ...
      backgroundImage: {
        ...
        'void-gradient': 'linear-gradient(135deg, #1e1b4b 0%, #312e81 50%,
      #1e293b 100%)',
      },
      animation: {
...
body {
  ...
  selection:bg-purple-500/30 selection:text-purple-200;
...
* **Approach A (Tailwind):** Apply `bg-linear-to-br from-slate-900
via-indigo-950 to-slate-900` to the app shell.
* **Approach B (CSS variable):** Define `--void-gradient` in `:root` and keep
`background-image: var(--void-gradient)`.
...
* **When to update:** whenever you introduce a new reusable pattern (new surface
, new button variant, new animation rule)
* **Avoid:** adding one-off rules that only apply to a single screen
* **Keep:** tokens and bundles small; delete outdated guidance instead of piling
on exceptions
