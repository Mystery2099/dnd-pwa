...
**Version:** 1.0.0
**Aesthetic:** "Modern Arcane Aero." A fusion of Frutiger Aero (gloss, depth, beveled glass) and modern product UI (clarity, restraint, accessibility).
...
### Consistency Contract
  ...
  * If a recommendation here conflicts with semantics, motion rules, or accessibility, the Design System wins.
  * Everything in this guide should map onto Design System tokens (surfaces, accents, states).
...
Our UI is not "flat." It is a tactile, three-dimensional artifact made of enchanted crystal and obsidian.

  * **Tactile Depth:** Elements should look like they can be touched. Buttons are convex (orbs), inputs are concave (insets).
  * **Luminous:** Everything emits light. Shadows are colored, not black. Text has halos.
  * **Vibrant:** Avoid washed-out accent colors. Use rich purples for backgrounds and gem tones for interactive emphasis (neutral grays are fine for typography).
  * **Reflective:** Surfaces are polished glass. They have "horizon lines" (the classic Aero gloss reflection).

### 1.1 Modern Constraints (So It Still Feels 2025)

Keep the Aero vibe, but avoid turning the UI into a neon toy.

  * **Glow is an accent, not the default:** reserve strong glows for selection, focus, or featured rarity.
  * **Bevels are subtle:** prefer 1-2 inset highlights, not many layers.
  * **Text glow is minimal:** the UI must remain readable; avoid heavy halos on body text.
...
Used for borders, buttons, badges, and small highlights. These are the primary "gem" hues.

  * **Semantics:** gemstone colors represent **spell school / spell role**, not rarity.
  * **Rarity:** should be expressed by **border/glow intensity** (and occasional pulse for featured items), not by changing the hue.
...
When an element is active, it fills with "liquid light."
  ...
  * **Behavior:** A radial gradient that follows the mouse (if possible) or pulses from the center.
...
Icons can be monochrome by default; use gem accents sparingly.
  ...
  * **Accent:** Use gem colors for school/semantic emphasis (e.g., Evocation gets a ruby accent), not everywhere.

### 7.2 Background Noise

To prevent the glass from looking like plastic, overlay a very faint noise texture.

  * **Asset:** Use a subtle transparent noise PNG over the entire `<body>`, set to `pointer-events-none`.
...
A long, horizontal bar that looks like a glass slat.
  ...
  * **Hover:** The "Aero Shine" gradient slides in. The left border lights up with the School Color (e.g., Red for Evocation).
...
Don't use a flat line. Use a **Glass Tube**.
  ...
  * **Fill:** A bright red cylinder with a white highlight running horizontally across the top half to make it look liquid.