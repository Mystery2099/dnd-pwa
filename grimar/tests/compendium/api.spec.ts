...
test.describe('Compendium API', () => {
	...
	test('should filter by level', async ({ request }) => {
		...
		const response = await request.get('/api/compendium/items?type=spells&spellLevel=1&limit=5', { headers });
		expect(response.status()).toBe(200);
		const body = await response.json();
	...
	test('should filter by search term', async ({ request }) => {
		...
		const response = await request.get('/api/compendium/items?type=spells&search=Fire&limit=5', { headers });
		expect(response.status()).toBe(200);
		const body = await response.json();
