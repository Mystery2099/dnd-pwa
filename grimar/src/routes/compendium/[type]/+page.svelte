...
>
	<div class="mx-auto max-w-[92rem] px-4 py-8">
		...
		>
			<div
				class="absolute inset-y-0 right-0 w-1/2 bg-[linear-gradient(120deg,transparent,color-mix(in_srgb,var(--color-accent)_8%,transparent),transparent)] opacity-70"
			></div>
			<div class="relative">
				<div>
					<Breadcrumb
					...
					<p
						class="mt-2 max-w-2xl text-[color-mix(in_srgb,var(--color-text-primary)_72%,var(--color-text-secondary))]"
					>
						{config.description}
					</p>
					<div class="mt-5 grid gap-3 sm:grid-cols-3">
						<div
							class="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-card)]/50 px-4 py-3"
						>
							<p
								class="text-[0.68rem] font-medium tracking-[0.18em] text-[var(--color-text-muted)] uppercase"
							>
								Entries
							</p>
		...
		<div class="grid gap-6 xl:grid-cols-[17rem_minmax(0,1fr)]">
			...
			>
				<div class="mb-5 flex items-start justify-between gap-4">
					<div>
						<p
							class="text-[0.68rem] font-medium tracking-[0.2em] text-[color-mix(in_srgb,var(--color-text-primary)_52%,var(--color-text-muted))] uppercase"
						>
							Refine Shelf
						</p>
			...
			<section class="space-y-6">
				<div class="flex flex-wrap items-end justify-between gap-3">
					<div>
						<p
							class="text-[0.72rem] font-medium tracking-[0.2em] text-[color-mix(in_srgb,var(--color-text-primary)_52%,var(--color-text-muted))] uppercase"
						>
							Browse Results
						</p>
						<p
							class="mt-1 text-sm text-[color-mix(in_srgb,var(--color-text-primary)_70%,var(--color-text-secondary))]"
						>
				...
				{#if isLoading}
					<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
						{#each Array(12) as _, index (index)}
							<div
								class="card-crystal h-40 animate-pulse rounded-[1.5rem] bg-[var(--color-bg-card)]"
							></div>
						{/each}
					</div>
				...
				{:else}
							...
							>
								<div class="relative p-4.5">
									...
									<div class="mb-2.5 flex items-center justify-between gap-3">
										<p
											class="text-[0.68rem] font-medium tracking-[0.18em] text-[color-mix(in_srgb,var(--color-text-primary)_48%,var(--color-text-muted))] uppercase"
										>
											{config.label}
										</p>
					...
					{#if totalPages > 1}
						<div
							class="mt-8 rounded-[1.5rem] border border-[var(--color-border)] bg-[var(--color-bg-card)]/35 p-4"
						>
