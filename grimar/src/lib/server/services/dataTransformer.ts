...
export class DataTransformer {
	 ...
	 */
	static transformCreature(creature: Open5eCreature): CreatureItem {
		log.debug({ key: creature.index, name: creature.name }, 'Transforming creature');

		const size = creature.size || 'Medium';
		const type = creature.type || 'Unknown';
		const crValue = creature.challenge_rating;
		...
		const hp = creature.hit_points ?? 0;

		log.debug({ key: creature.index, name: creature.name, cr }, 'Creature transformed');
		...
		return {
			...
			hit_points: hp,
			hit_dice: creature.hit_dice || '1d8',
			strength: 10,
			dexterity: 10,
			constitution: 10,
			intelligence: 10,
	 ...
	 */
	static createSummary(item: Open5eSpell | Open5eCreature, type: 'spell' | 'creature'): string {
		if (type === 'spell') {
			...
			const creature = item as Open5eCreature;
			const size = creature.size || 'Medium';
